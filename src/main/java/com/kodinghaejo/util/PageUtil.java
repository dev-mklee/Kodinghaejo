...
public class PageUtil {
	...
	//화면에 하나의 페이징만 필요할 때 사용
	public String getAdminMemberPageList(int pageNum, int postNum, int pageListCount, int totalCount, String searchType, String keyword) {
		//pageNum : 현재 페이지 번호
		//postNum : 한 화면에 보여지는 게시물 행 갯수
		//pageListCount : 하단 페이지리스트에 보여질 페이지 갯수
		//totalCount : 전체 행갯수
		...
		if (totalPage > 1) {
			...
			for (int i = 1; i <= pageListCount; i++) {
				...
				if (i == 1) {
					...
					//<<(첫 페이지로 이동) 출력
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=1&searchType=" + searchType + "&keyword=" + keyword + "\" class=\"left-prev-arrow\">\n"
										+ "\t\t<svg width=\"12\" height=\"12\" viewBox=\"0 0 22 22\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n"
										+ "\t\t\t<path d=\"M11 1L1 11.1579L10.6891 21\" stroke=\"black\" stroke-linecap=\"round\"></path>\n"
										+ "\t\t\t<path d=\"M21 1L11 11.1579L20.6891 21\" stroke=\"black\" stroke-linecap=\"round\"></path>\n"
										+ "\t\t</svg>\n"
					...
					//<(이전 페이지로 이동) 출력
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=" + Integer.toString(prevPage) + "&searchType=" + searchType + "&keyword=" + keyword + "\" class=\"left-arrow\"><i class=\"xi-angle-left-thin\"></i></a>\n";
				}
				//2. 페이지 출력 중단
				if (totalPage < (section - 1) * pageListCount + i) {
					int nextPage = (totalPage >= i + (section - 1) * pageListCount + 1) ? (section * pageListCount + 1) : totalPage;
					...
					//>(다음 페이지로 이동) 출력
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=" + Integer.toString(nextPage) + "&searchType=" + searchType + "&keyword=" + keyword + "\" class=\"right-arrow\"><i class=\"xi-angle-right-thin\"></i></a>\n";
					
					//>>(마지막 페이지로 이동) 출력
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=" + Integer.toString(lastPage) + "&searchType=" + searchType + "&keyword=" + keyword + "\" class=\"right-next-arrow\">\n"
										+ "\t\t<svg width=\"12\" height=\"12\" viewBox=\"0 0 22 22\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n"
										+ "\t\t\t<path d=\"M11 1L21 11.1579L11.3109 21\" stroke=\"black\" stroke-linecap=\"round\"></path>\n"
										+ "\t\t\t<path d=\"M1 1L11 11.1579L1.31088 21\" stroke=\"black\" stroke-linecap=\"round\"></path>\n"
										+ "\t\t</svg>\n"
				...
				//3. 인자로 가져 온 페이지 값과 계산해서 나온 페이지 값이 같으면 해당 페이지 활성상태 표시, 링크는 무조건 붙임.
				if (pageNum != (section - 1) * pageListCount + i)
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=" + Integer.toString((section - 1) * pageListCount + i) + "&searchType=" + searchType + "&keyword=" + keyword + "\">" + Integer.toString((section - 1) * pageListCount + i) + "</a>\n";
				else
					pageList += "\t<a href=\"/admin/systemMemberInfo?page=" + Integer.toString((section - 1) * pageListCount + i) + "&searchType=" + searchType + "&keyword=" + keyword + "\" class=\"on\" role=\"button\">" + Integer.toString((section - 1) * pageListCount + i) + "</a>\n";
				//4. >, >> 출력
				// totalPage >= i + (section - 1) * pageListCount + 1 --> 아직까지 출력할 페이지가 남아 있음.
				if (i == pageListCount) {
					// && totalSection > 1 && totalPage >= i + (section - 1) * pageListCount + 1
